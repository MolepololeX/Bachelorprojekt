...
@export var draw_mode : DrawMode = DrawMode.none

#@export_tool_button("Capture Before", "CameraTexture") var enable_capture_action = _enable_capture
...
var capture_frame_delay = 300

var texture : StringName = "texture_before"
var texture2 : StringName = "texture_after"
...
func _generate_post_image()->void:
	...
	#format.format = RenderingDevice.DATA_FORMAT_R8G8B8_USCALED
	...
	# Create uniform (binding = 0 in shader)
	...
	uniform_before.add_id(dummy_tex)
	...
	if rd  and shader_is_valid:
		...
		if render_scene_buffers:
			...
			if !render_scene_buffers.has_texture(context, texture):
				...
				render_scene_buffers.create_texture(context, texture2, RenderingDevice.DATA_FORMAT_R16G16B16A16_SFLOAT, usage_bits, RenderingDevice.TEXTURE_SAMPLES_1, size, 1, 1, true, false)
			...
			for view in range(view_count):
				...
				if(enable_capture_pre):
					...
					img.save_png("res://_before.png")
					img.convert(Image.FORMAT_RGB8)
					img.resize(size.x*2, size.y*2, Image.INTERPOLATE_NEAREST)
					img.linear_to_srgb()
					img.save_png("res://_before_png.png")
				...
				#texture_image = render_scene_buffers.get_texture_slice(context, texture2, view, 0, 1, 1)
				#var uniform_after := RDUniform.new()
				#uniform_after.uniform_type = RenderingDevice.UNIFORM_TYPE_IMAGE
				#uniform_after.binding = 3
				#uniform_after.add_id(texture_image)
